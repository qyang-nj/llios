...
Another intersting thing is that the version number is encoded in a 32-bit integer (16 bits for major version, 8 bits for minor version and 8 bits for patch version), so **the maxium of minor version is 15**. This's probably why Apple decided to set macOS version from 10.15 (Catalina) straigt to 11 (Big Sur), after being version 10.x for about twenty years.
